    ...
    if nround == 1:
        ...
        for group in subsession.get_groups():
            ...
            for p in group_players:
                ...
                p.participant.vars['rfirst'] = bool(random.getrandbits(1))
            if group_gen > 0:
        ...
        #active_players = [player for player in players if player.session_gen == subsession.current_gen]
            ...
            if p.session_gen < p.subsession.current_gen:  # if retired, recall advice, survey generations, increment stage
                ...
                if p.stage <= p.subsession.num_questions:
                    p.r_survey = p.participant.vars['rfirst']
                else:
                    p.r_survey = not p.participant.vars['rfirst']
...
def get_tag(player: Player): # identify opponent
    ...
    if side:
        ...
        tag += 'B'
    player.participant.vars['tag'] = tag
...
class Introduction(Page):
    ...
    def vars_for_template(player: Player):
        if player.session_gen > 1:
            pred = get_predecessor(player)
            player.participant.vars['r_advice'] = pred.participant.vars['g_advice']
            player.ancestor_participant_id = pred.participant_id
        try:
            if player.r_survey:
                advice = player.participant.vars['r_advice']
            else:
                advice = player.participant.vars['g_advice']
        except:
            advice = ''
        return {
...
class Survey(Page):
    ...
    def vars_for_template(player: Player):
        ...
        if r_survey:
            ...
            active_tag = player.participant.vars['tag']
            opp_tag = get_opponent(player).participant.vars['tag']
        else:
        ...
        return {
            ...
            'asurvey': player.participant.vars['asurvey'],
            'rfirst': player.participant.vars['rfirst'],
            'history':historyq,
            'first':historyq == {},
            'delta':player.subsession.delta,
            'CTAG':C.C_TAG,
...
class ResultsWaitPage(WaitPage):
        ...
        for group in subsession.get_groups():
            ...
            for p in group.get_players():
                ...
                if p.session_gen > 1:
                    num_question = 2*N
                    predecessor = get_predecessor(p)
                    if predecessor.participant.vars['rfirst']:
                ...
                if p.session_gen != 1:
                    ...
                    if Rdone and Gdone:
                        ...
                        p.participant.vars['rpoints'] = r_advice_score(p)
                        if p.session_gen > 1:
                            predecessor.participant.vars['gpoints'] = p.participant.vars['rpoints']
                            predecessor.final_period = predecessor.participant.vars['wait']
                if p.session_end:
                    p.participant.vars['gpoints'] = 0
            ...
            for p in group.get_players():
                if p.subsession.stop_session:
                    p.payoff += p.participant.vars['gpoints'] + p.participant.vars['rpoints'] # payoff from interpreting
    ...
    def vars_for_template(player: Player):
        ...
        if player.stage == player.subsession.num_questions:
            player.r_survey = not player.r_survey
            if player.r_survey:
                advice = player.participant.vars['r_advice']
            else:
                advice = player.participant.vars['g_advice']
        return {
            ...
            'succ_tag': successor_tag(player),
            'survey_instr': player.stage == player.subsession.num_questions and player.session_gen != 1,
...
class SaveLocal(WaitPage):
    ...
    def is_displayed(player: Player):
        N = player.subsession.num_questions
        return (player.subsession.last_period and not player.session.config['gen_end'] and player.stage <= 0) or\
            player.stage == player.subsession.num_questions

    @staticmethod
    def vars_for_template(player: Player):
        opp = get_opponent(player)
        player.r_survey = player.participant.vars['rfirst']
        if player.stage == player.subsession.num_questions:
            player.r_survey = not player.r_survey
        if player.r_survey:
            advice = player.participant.vars['r_advice']
            ...
            advice = player.participant.vars['g_advice']
        return {
            'inactive':player.stage<0,
            'r_advice': player.participant.vars['r_advice'],
            'rsurvey':player.r_survey,
            'survey_instr': (player.stage == 0 or player.stage == player.subsession.num_questions),
            'stage':player.stage,
            'pred_tag': predecessor_tag(player),
            'pred_opp_tag': predecessor_tag(opp),
            'advice': advice,
            'tag': player.participant.vars['tag'],
            'opp_tag': opp.participant.vars['tag'],
            'succ_tag': successor_tag(player),
...
class SaveSession(WaitPage):
    ...
    def app_after_this_page(player: Player, upcoming_apps):
        player.participant.vars['rsurvey'] = sort_rresponse(player)
        player.participant.vars['gsurvey'] = sort_gresponse(player)
        player.g_advice = player.participant.vars['g_advice']
...
class EndOfGame(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            ...
            gpoints= player.participant.vars['gpoints'],
...
class EndWait(WaitPage):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return player.final_period and not player.session_end
